...
ookburst::ookburst(uint64_t TuneFrequency, float SymbolRate, int Channel, uint32_t FifoSize, size_t upsample, float RatioRamp) :
        bufferdma(Channel, FifoSize * upsample + 2, 2, 1), SR_upsample(upsample) {
        ...
        for (size_t j = 0; j < Ramp; j++) {
            if (i < Size - 1) {
                sampletab[i * SR_upsample + j + SR_upsample - Ramp] =
                        (Symbols[i] == 0) ? ((Originfsel & ~(7 << 12)) | (0 << 12)) : ((Originfsel & ~(7 << 12)) | (4 << 12));
...
ookbursttiming::ookbursttiming(uint64_t TuneFrequency, size_t MaxMessageDuration) :
        ookburst(TuneFrequency, 1e5, 14, MaxMessageDuration / 10) {
